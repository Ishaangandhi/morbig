...
module type Lexer =
  sig
    val initialize: prelexer_state -> Lexing.lexbuf -> (unit -> unit) -> unit
...
let parse_loop partial (module Lexer : Lexer) state =
        ...
        | Some false ->
           ...
           let csts =
             if Options.(backend () = NoSerialisation) then
               ...
               cst :: csts
           in
           if Options.interactive () then csts, { aliases; checkpoint } else
...
let parse partial (module Lexer : Lexer) =
  let csts, _state = parse_loop partial (module Lexer) None in
  csts
...
module Lexer (U : sig end) : Lexer = struct
  ...
  let on_ps2 = ref (fun () -> assert false)
  ...
  let initialize current lexbuf _on_ps2 =
...
let rec parse_interactively on_ps2 partial current lexbuf state =
  let subshell_parser c l =
    Options.disable_interactive ();
    let _, _, csts = parse_interactively on_ps2 true c l None in
    Options.enable_interactive ();
    csts
  in
  RecursiveParser.parse := subshell_parser;
  let module Lexer = Lexer (struct end) in
  Lexer.initialize current lexbuf on_ps2;
  let csts, new_state = parse_loop partial (module Lexer) state in
  let merged_cst = ExtPervasives.reduce CSTHelpers.empty_program CSTHelpers.concat_programs (List.rev csts) in
  (lexbuf, new_state, merged_cst)

let parse ?(on_ps2 = fun () -> ()) partial current lexbuf =
  let module Lexer = Lexer (struct end) in
  Lexer.initialize current lexbuf on_ps2;
